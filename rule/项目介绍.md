...
- **pnpm**: 8.0.0 或更高版本（推荐使用pnpm作为包管理器）
...
1. **TypeScript优先**: 使用TypeScript类型定义确保代码类型安全
2. **组件化开发**: 遵循组件化开发原则，保持组件的单一职责
3. **文件命名**: 严格遵循项目文件命名规则
4. **代码注释**: 使用中文注释，简洁明了地说明复杂逻辑
...
1. **现代化技术栈**: 使用最新的React 19、Vite 7、TypeScript等
2. **完整认证系统**: 开箱即用的管理员登录和权限管理
3. **统一API处理**: 完善的HTTP拦截器和错误处理机制
4. **类型安全**: 全面的TypeScript类型定义和检查
5. **开发体验**: 热更新、ESLint检查、完善的开发工具链
6. **UI组件丰富**: 50+个高质量的shadcn/ui组件
7. **响应式设计**: 支持各种设备尺寸的响应式布局
8. **主题系统**: 完整的亮色/暗色主题切换功能